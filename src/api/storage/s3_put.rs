...
pub async fn put_bucket_object(
    ...
    if super::s3_tagging::is_tagging_subresource(raw_query.as_deref()) {
        return handle_put_tagging(&state, &bucket, &key, &scoped_bucket, &body).await;
    }
...
fn parse_put_headers(
    headers: &HeaderMap,
    body: &Bytes,
    bucket: &str,
    key: &str,
) -> Result<PutHeaders, Response> {
    ...
    let checksum_header = super::extract_checksum_header(headers).map_err(|message| {
        s3_error_response(
            StatusCode::BAD_REQUEST,
            "InvalidRequest",
            &message,
