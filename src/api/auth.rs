...
pub async fn register(
    ...
    match result {
        Ok(_) => (StatusCode::CREATED, "User registered. Wait for admin approval if not the first user.").into_response(),
...
pub async fn login(
    ...
    let user: Option<User> = sqlx::query_as("SELECT id, email, password_hash, is_active, is_admin FROM users WHERE email = ?")
        .bind(&payload.email)
        .fetch_optional(&pool)
        .await
        .unwrap();
    ...
    if let Some(user) = user {
        ...
        if crate::auth::hash::verify_password(&payload.password, &user.password_hash) {
            let token = crate::auth::jwt::create_jwt(&user.id, user.is_admin, "temp_secret");
